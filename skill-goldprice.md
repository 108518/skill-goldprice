...
### 方案一：使用 Playwright (推荐)

由于目标网站是使用JavaScript动态渲染的单页应用(SPA)，需要使用浏览器自动化工具来获取实时数据。

```javascript
// 使用 Playwright 获取金价数据
...
async function getGoldPrices() {
  ...
  try {
    ...
    // 等待价格数据加载
    await page.waitForSelector('.price-table-row', { timeout: 10000 });
...
### 方案二：使用 Playwright MCP Tool

如果在支持Playwriter MCP的环境中使用，可以直接使用：
...
网站可能通过API获取数据，但需要进一步抓包分析。常见的API端点可能包括：

```
https://i.jzj9999.com/api/quote
https://i.jzj9999.com/api/v1/market
```

**注意**: 这些端点可能需要特定的请求头、认证或WebSocket连接。
...
**字段说明**:
- `name`: 贵金属品种名称（如 Au99.99、Au100g、Ag99.9 等）
- `bidPrice`: 回购价格
- `askPrice`: 销售价格
- `high`: 当日最高价
- `low`: 当日最低价
...
```javascript
const prices = await getGoldPrices();
const goldPrices = prices.filter(p => p.name.startsWith('Au'));
...
async function checkPriceAlert(targetPrice) {
  const prices = await getGoldPrices();
  const gold = prices.find(p => p.name === 'Au99.99');
  
  if (gold && parseFloat(gold.bidPrice) >= targetPrice) {
    console.log(`价格提醒: Au99.99回购价达到 ${gold.bidPrice}`);
  }
}
```
...
1. **网络要求**: 需要能够访问外网
2. **浏览器资源**: 运行Playwright需要一定的系统资源
3. **稳定性**: 网站结构可能变化，需要定期维护选择器
4. **合规性**: 遵守网站的使用条款和robots.txt规则
